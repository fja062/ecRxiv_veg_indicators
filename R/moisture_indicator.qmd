...
# check name changes
tyler_species_clean |> filter(clean_string != accepted_name) |> view()
...
ASO_species_clean <- left_join(ASO_prepared, ASO_sp_clean, by = "clean_string") |> 
  ...
  distinct() |> 
  # add on Hieracium Vulgata group
  mutate(
    accepted_name = case_when(
...
summary(ASO_all)
 ...
 #rm(ASO_species, ASO_species_ind, ASO_prepared, ASO_sp_matched, ASO_points, ASO_sp_clean, ASO_species_clean, ASO_species_prepared, ASO_prepared_wfo)
...
#forest.ref.cov <- indBoot.freq(sp=NiN.forest.cov[,1],abun=NiN.forest.cov[,2:27],ind=NiN.forest.cov[,28:29], iter=1000,obl=0.8,rat=1/2,var.abun=T)
