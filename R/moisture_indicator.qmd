...
GRUK_species_ind <- GRUK_species_clean |>
  select(accepted_name, art_dekning, parent_global_id, polygon_id, rute_id) |> 
...
unique(GRUK_species_ind[is.na(GRUK_species_ind$Moisture & 
                                is.na(GRUK_species_ind$Nitrogen)),'accepted_name'])# |> view()
...
unique(ANO_species_ind[is.na(ANO_species_ind$Moisture & 
                               is.na(ANO_species_ind$Nitrogen)),'accepted_name']) # |> view()
...
nin_sp_ind <- nin_species_clean |>
  ...
  left_join(tyler_species_clean |> 
              select(species = accepted_name, Moisture, Nitrogen),
            by = join_by(species)) |> 
  tibble()
...
nin_sp_ind |> filter(is.na(Moisture)&is.na(Nitrogen)) |>  distinct(species)  # some species to sort here.
...
nin_doubles <- colnames(lowland_ref_cov[["Moisture"]]) |> str_subset("[abcd]$")

# remove a-b-c-d subtypes
colnames(lowland_ref_cov[["Moisture"]]) <- colnames(lowland_ref_cov[["Moisture"]]) |> 
  str_remove("[abcd]$")
...
nin_cols    <- names(lowland_ref_cov[["Moisture"]]) |> 
  str_subset("^(T|V)[0-9]")
