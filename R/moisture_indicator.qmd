...
# 2.4 reference data - data handling
...
y.Moist[((1:dim(tab)[1])*2)-1] <- tab$Moisture_025 
y.Moist[((1:dim(tab)[1])*2)] <- tab$Moisture_975 

y.Moisture <- numeric(length=nrow(tab)*2)
y.Moisture[((1:dim(tab)[1])*2)-1] <- tab$Moisture_025_dup 
y.Moisture[((1:dim(tab)[1])*2)] <- tab$Moisture_975_dup 
...
lowland_ref_cov_val <- lowland_ref_cov_val |> tibble() |> 
...
gruk_lowlands <- GRUK_all |>
  mutate(
    X = st_coordinates(GRUK_all)[, 1],
    Y = st_coordinates(GRUK_all)[, 2],
    kartleggingsenhet_1m2 = word(kartleggingsenhet_1m2, 1, 1)
  ) |>
  st_drop_geometry() |> 
  mutate(globalid_chr = as.character(globalid)) %>%
